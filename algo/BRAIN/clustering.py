...
class LogParser:
    ...
    def parse(self, log, logname='Linux'):
        self.log = log
        ...
        group_len, tuple_vector, frequency_vector = self.get_frequecy_vector(
            sentences, self.rex, self.delimeter, logname
        )
        ...
        return self.get_out(template_set, sentences)


    def runClassifyLog(self, log, modelID, logname='Linux'):
        self.log = log
        starttime = datetime.now()
        self.load_data()
    ...
    def get_out(self, template_set, sentences):
        ...
        out_st = [
            {"LineId": l[0], "Date": l[1], "Time": l[2], "Pid": l[3], "Level": l[4], "Component": l[5], "Content": l[6],
             "EventId": l[7], "EventTemplate": l[8], "ParameterList": l[9]} for l in
            zip(self.df_log["LineId"], self.df_log["Date"], self.df_log["Time"], self.df_log["Pid"],
                self.df_log["Level"], self.df_log["Component"], self.df_log["Content"], self.df_log["EventId"],
                self.df_log["EventTemplate"], self.df_log["Content_"])]

        out_tm = [{"EventId": l[0], "EventTemplate": l[1], "Occurrences": l[2]} for l in
                  zip(df_event["EventId"], df_event["EventTemplate"], df_event["Occurrences"])]
        return out_st, out_tm
    ...
    def generateresult(self, template_set, sentences):
        ...
        self.df_log["EventTemplate"] = template_
        self.df_log.to_csv(
            os.path.join(self.savePath, "test" + "_structured.csv"), index=False
        )
        ...
        df_event.to_csv(
            os.path.join(self.savePath, "test" + "_templates.csv"),
            index=False,
    ...
    def load_data(self):
        ...
        headers, regex = self.generate_logformat_regex(self.logformat)
        self.df_log = self.log_to_dataframe(
            self.log, regex, headers, self.logformat
        )
    ...
    def log_to_dataframe(self, log, regex, headers, logformat):
...
if __name__ == '__main__':
    ...
    logs = [
        "2024-05-27 12:00:00 1234 INFO Component1: This is an info message",
        "2024-05-27 12:01:00 1235 ERROR Component2: This is an error message",
        "2024-05-27 12:02:00 1236 DEBUG Component3: This is a debug message",
        "2024-05-27 12:03:00 1237 WARNING Component4: This is a warning message",
    ]
